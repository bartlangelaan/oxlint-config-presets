...
This package lives at `packages/oxlint-config-presets` in the [oxlint-config-presets](https://github.com/popup-plus/oxlint-config-presets) monorepo. Run the commands below from this directory, or prefix them with `pnpm --filter oxlint-config-presets` from the repo root.
